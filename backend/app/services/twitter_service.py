...
def update_user_avatars(db: Session, usernames: List[str]) -> None:
    try:
        for i in range(0, len(usernames), 100):
            ...
            response = twitter_client.get_users(
                usernames=batch, 
...
def get_avatars_for_usernames(usernames: List[str], db: Session) -> Dict[str, str]:
    ...
    existing_users = db.query(User).filter(
        func.lower(User.twitter_username).in_([u.lower() for u in usernames])
    ).all()
    
    to_update = {
        u.twitter_username for u in existing_users 
        if not u.avatar_updated_at or u.avatar_updated_at < cutoff
    } | {
        u for u in usernames 
        if u.lower() not in {eu.twitter_username.lower() for eu in existing_users}
    }
...
def post_reply_to_twitter_with_comment(db: Session, tip: Tip) -> None:
    ...
    reply_text = f"⚡{comment}⚡ {tipper_username} tipped you {tip.amount_sats} sats"
